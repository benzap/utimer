(defproject utimer "0.1.0-SNAPSHOT"
  ...
  :uberjar-name "utimer.jar"

  ;; Use `lein run` if you just want to start a HTTP server, without figwheel
  :main utimer.application
  ...
  :cljsbuild {:builds
              [{:id "app"
                ...
                :figwheel {:on-jsload "utimer.system/reset"}
  ...
  :profiles {:dev
             ...
             {:source-paths ^:replace ["src/clj" "src/cljc"]
              :prep-tasks ["compile"
                           ["cljsbuild" "once" "min"]
                           ["run" "-m" "garden-watcher.main" "utimer.styles"]]
              :hooks []
              :omit-source true
              :aot :all}})
